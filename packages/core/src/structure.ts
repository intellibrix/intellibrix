...
export default class Structure {
   ...
   */
  add(brick: Brick) {
    if (this.#bricks.find((b) => b.name === brick.name || b.id === brick.id))
      throw new Error('Brick already exists in structure')
  ...
  remove(brick: Brick) {
    if (!this.#bricks.find((b) => b.name === brick.name || b.id === brick.id))
      throw new Error('Brick does not exist in structure')
